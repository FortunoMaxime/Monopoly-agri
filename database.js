  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const params = [
        ...
        row[11], // Novembre
        row[12] ,
        id , // Decembre (la dernière colonne n'est pas incluse)
      ];
...
async function insertTotaly(data, nomtable,mpamokatraid) {
  ...
  for (let i = 0; i < data.length; i++) {
   ...
   // values.push(mpamokatraid);
    try {
      await db.runAsync(`
        INSERT INTO ${nomtable}(Etiquete, Janvier, Fevrier, Mars, Avril, Mai, Juin, Juillet, Aout, Sepetembre, Octobre, Novembre, Descembre) 
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        values.join(',') // Spécifiez ',' comme délimiteur
      );
